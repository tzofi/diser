...
def auto_annotate(vehicles, camera, depth_img, max_dist=1000, depth_margin=-1, patch_ratio=0.5, resize_ratio=0.5, json_path=None):
    depth_show = False
    #print(len(vehicles))
    _, idxs = filter_angle_distance(vehicles, camera, max_dist)
    #print(len(vehicles))
    ...
    if json_path is not None:
        ...
        vehicle_class = []
    filtered_out, removed_out, _, _, filtered = filter_occlusion_bbox(bounding_boxes_2d, vehicles, camera, depth_img, vehicle_class, depth_show, depth_margin, patch_ratio, resize_ratio)
    for idx in range(len(filtered)):
        if idx not in idxs:
            filtered[idx]=False
    
    return filtered_out, removed_out, filtered 
...
def filter_angle(vehicles_list, v_transform, v_transform_s, sensor):
    ...
    idxs = [idx for idx, (v, s) in enumerate(zip(vehicles_list, selector)) if s]
    v_transform_f = v_transform[selector[:,0],:]
    v_transform_s_f = v_transform_s[selector[:,0],:]
    return vehicles_list_f , v_transform_f , v_transform_s_f, idxs
...
def filter_angle_distance(vehicles_list, sensor, max_dist=100):
    ...
    vehicles_list , vehicles_transform , vehicles_transform_s, idxs = filter_angle(vehicles_list, vehicles_transform, vehicles_transform_s, sensor)
    return vehicles_list, idxs
...
def filter_occlusion_bbox(bounding_boxes, vehicles, sensor, depth_img, v_class=None, depth_capture=False, depth_margin=-1, patch_ratio=0.5, resize_ratio=0.5):
    ...
    dont_filter = [True for _ in bounding_boxes]
    i = 0
    ...
    return filtered_out, removed_out, patches, depth_capture, dont_filter
