...
def add_npcs(world, nnpcs, pos_agents, ego_start, pos_inits, car_color_range, og_color,
    ...
    # ego_x, ego_y, ego_z = ego_start.location.x, ego_start.location.y, ego_start.location.z
    ...
    # while num_added < nnpcs and place_iter < len(circles):
    #     try:
    #         place_iter += 1
    ...
    #         # set loc 
    #         loc = carla.Location(ego_x + circles[place_iter][0],
    #                              ego_y + circles[place_iter][1],
    #                             ego_z + 2)
    #         location = pos_inits[place_iter]
    #         map = world.get_map()
    ...
    ixes = sorted(range(len(pos_inits)), key=lambda ix: dists[ix] if dists[ix] > 2 else 10000)
...
def init_env(host, port, tm_port, width, height, timeout, calibf, ncarcalib, rnd_seed,
    ...
    all_x = [tr.location.x for tr in pos_inits]
    all_y = [tr.location.y for tr in pos_inits]
    #print("minx: ", min(all_x))
    #print("maxx: ", max(all_x))
    #print("miny: ", min(all_y))
    #print("maxy: ", max(all_y))
    # pos_inits = list(filter(lambda tr: (tr.location.y < 0), pos_inits)) 
    # pos_inits = [list(filter(lambda tr: (tr.location.y < 0), pos_inits)) ]
