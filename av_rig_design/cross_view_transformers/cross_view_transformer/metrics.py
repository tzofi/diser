...
class BaseIoUMetric(Metric):
    ...
    def __init__(self, thresholds=[0.4, 0.5]):
        ...
        thresholds = torch.FloatTensor(thresholds).to(self.dev)
    ...
    def update(self, pred, label):
        self.thresholds = self.thresholds.to(self.dev)
        pred = pred.to(self.dev)
        label = label.to(self.dev)
        self.tp = self.tp.to(self.dev)
        self.fp = self.fp.to(self.dev)
        self.fn = self.fn.to(self.dev)
